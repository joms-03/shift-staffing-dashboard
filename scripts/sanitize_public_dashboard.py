...
def main() -> None:
    bf5_path = Path("ops-portal-a7c93e4b16f28d05c4e9713b/bf5.html")
    ...
    if PHONE_PATTERN.search(html):
        raise SystemExit(f"Refusing to write: phone-like Pro data remains in {bf5_path}")

    bf5_path.write_text(html, encoding="utf-8")

    vip_path = Path("ops-portal-a7c93e4b16f28d05c4e9713b/vip.html")
    vip_html = vip_path.read_text(encoding="utf-8")
    if PHONE_PATTERN.search(vip_html):
        raise SystemExit(f"Refusing to publish: phone-like Pro data remains in {vip_path}")
